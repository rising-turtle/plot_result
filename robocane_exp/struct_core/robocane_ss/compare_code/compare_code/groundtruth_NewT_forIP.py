...
def CleanData(errorT,data):
	...
	potentialPointsNum = (potentialPointsNum-2)/3;
	...
	for i in range(pointsLength):
	    for j in range(potentialPointsNum):
	    	ppdistance = np.linalg.norm( allPoint[i,:,:] - (allPoint[i,:,j]*(np.ones((potentialPointsNum,1)))).T ,2,0);
	        ...
	        if ( len(check045_result[0]) and len(check075_result[0]) and len(check099_result[0]) and len(check110_result[0])and len(check115_result[0])):
	            ...
	            laserDistance = np.linalg.norm( allPoint[i,:,check045_result[0][0]] - allPoint[i,:,check110_result[0][0]] ,2,0)
	...
	cleanData = np.concatenate((markers[:,:,0], markers[:,:,1],markers[:,:,2],markers[:,:,3],markers[:,:,4],markers[:,:,5]),axis = 1)
...
if __name__ == "__main__":
	...
	T_led2b[0:3,0:3] = np.array(np.array(r.as_dcm()))
	...
	np.savetxt(gt_path, groundTruth, delimiter=' ', fmt=('%10.9f', '%2.6f', '%2.6f', '%2.6f', '%2.6f', '%2.6f', '%2.6f', '%2.6f'))
